...
env_names = ["ant4-walk-v0", "ant3-walk-v0"]
...
log_dir = f'runs/{datetime_st}_StructuredVAE_'
...
for epoch in range(args.epochs):
    ...
    for iteration in range(int(len(dataset) / args.batch_size)):
        ...
        if epoch > args.warmup_epochs:
            ...
            for _ in range(args.discriminator_times):
                ...
                disc_loss, disc_acc = vae_model.train_discriminator(x1, x2, x3, structure_3, epoch)
    ...
    writer.add_scalar('Generator/generator_loss', 
                      (gen1_tot + gen2_tot + beta * kl_tot) / n_batches, epoch)
    writer.add_scalar('Generator/generator_loss1', 
                      (gen1_tot) / n_batches, epoch)
    writer.add_scalar('Generator/generator_loss2', 
