...
parser.add_argument('--env2-name', default="ant3",
                    help='Mujoco Gym environment (default: HalfCheetah-v2)')
parser.add_argument('--vae_path', default="runs/2021-05-19_13-46-41_VAE_ant-v0_both/",
                    help='model path')
...
model.load_model(args.vae_path)

def pad_state(data, state_size, max_num_limbs):
    max_dim = ROOT_DIM + state_size * (max_num_limbs - 1)
...
with imageio.get_writer(args.video_file_name, fps=30) as video:
    ...
    for i_episode in itertools.count(1):
        ...
        while not done:
            ...
            x_hat = model((state, src_topology, tgt_topology))
            x_hat = x_hat.detach().cpu()
            x_hat = x_hat[:(render_limbs-1)]
            x_hat = np.concatenate([x_root, x_hat.ravel()], axis=-1)
            x_hat = torch.FloatTensor(x_hat).to(device).unsqueeze(0)

            action, _, _ = policy.sample(x_hat)
            action = action.detach().cpu().numpy()[0]
            next_state, reward, done, _ = render_env.step(action[:6])
            video.append_data(render_env.render('rgb_array'))
